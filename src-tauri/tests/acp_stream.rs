...
use packetcode_gui_lib::engine::{
    ...
    EngineState, McpServerStatus, PromptOutcome, SessionUsage, PERMISSION_MODES,
};
...
async fn graceful_stop_lets_the_engine_run_its_own_shutdown() {
    ...
    let session = new_session_on(&state, ".", None, None, None, false).await.unwrap();
...
async fn stop_kills_an_engine_that_ignores_stdin_close() {
    ...
    new_session_on(&state, ".", None, None, None, false).await.unwrap();
...
async fn abandoned_permission_waiter_is_reaped_when_its_turn_ends() {
    ...
    let (state, mut rx) = start_mock().await;
    let a = new_session_on(&state, ".", None, None, None, false).await.unwrap();
    let b = new_session_on(&state, ".", None, None, None, false).await.unwrap();
    assert_ne!(a, b);
